...
TEST_LOG_FILE="${PROJECT_DIR}/build/providers-reauth-ui-smoke.log"
...
launch_dev_app() {
    ...
    log_step "Launching ${DEV_PRODUCT_NAME} with providers smoke hook..."
    env \
        ...
        QUOTIO_UI_SMOKE_PROVIDERS_REAUTH=1 \
        "${DEV_EXECUTABLE}" \
...
print_header "Providers Reauth UI Smoke" 50
...
if defaults export "${DEV_BUNDLE_ID}" - >/dev/null 2>&1; then
    TEST_DEFAULTS_BACKUP="$(mktemp "${TMPDIR:-/tmp}/quotio-providers-ui-smoke-defaults.XXXXXX.plist")"
...
log_step "Waiting for providers reauthentication smoke markers..."
wait_for_log_line "[ui-smoke] providers-reauth-sheet-ready"
wait_for_log_line "[ui-smoke] providers-reauth-started"
wait_for_log_line "[ui-smoke] providers-reauth-url-ready"
wait_for_log_line "[ui-smoke] providers-reauth-cancelled"

print_summary "Smoke Passed" \
    "Account Settings" "auto-opened" \
    "Reauth Flow" "sheet -> start -> url-ready -> cancel" \
